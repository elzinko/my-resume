...
**Référence canonique** des **invariants du rendu du CV** : ce qui doit rester vrai
après chaque feature. Consolide le digest auto-chargé de [`CLAUDE.md`](../CLAUDE.md), la
décision de l'[ADR-0001](adr/0001-cv-rendering-regimes.md) et la
[checklist de revue](cv-rendering-review-checklist.md) — en cas de doute, **ce document
...
## 0. Le modèle mental — 1 DOM, 4 rendus + mobile
...
→ **4 rendus à garder cohérents** (`full-web`, `full-print`, `short-web`,
`short-print`) **+ le mobile**.
...
| 1.3 | **Garde-fou mobile du court.** Les règles `.cv-print-preview .cv-short-page` imposant des tailles A4 sont bornées `@media (min-width: 768px)`. Sous `md`, le court est une **vue mobile 1:1** (`zoom = 1`, typo lisible), jamais un A4 réduit. | short-web mobile | 🧪     |
...
| #   | Règle                                                                                                                                                                                               | Rendus     | Statut           |
| --- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ---------------- |
| 2.1 | **Photo affichée sur desktop, masquée sur mobile** (< 768px → toute la largeur pour le nom).                                                                                                        | full-web   | ✅               |
| 2.2 | **Photo TOUJOURS visible à l'impression**, quelle que soit la marge `@page` (garde-fou `print:!flex` : sans lui, une marge large fait passer la largeur sous 768px et `max-md:hidden` la masquait). | full-print | ✅               |
...
| 3.2 | **Mobile : Profil + domaines = un seul bloc groupé** — rythme vertical UNIFORME (`fin texte précédent → titre` == `titre → texte`, ≈ 16px) pour Summary→Agile et entre Agile/Dev/Ops ; l'écart vers la vraie section suivante reste nettement plus grand. Posé en `@media screen` (JAMAIS en print → 3 colonnes du PDF intactes même <768px), scopé `.cv-full-cv-print-root` (CV court non touché). | full mobile | 🧪     |
| 3.3 | **Print (3 colonnes) : les 5 pastilles de chaque domaine tiennent sur 1 SEULE ligne**, taille compacte identique (10px), sans rétrécir la police en-deçà du lisible. Vérifié PDF réel (786px) + aperçu 768→1024px.                                                                                                                                                                                  | full-print  | ✅     |
...
| 3.5 | Web/mobile : pastilles à 13-14px (lisibles), **non impactées** par la compaction print.                                                                                                                                                                                                                                                                                                             | full-web    | ✅     |
...
| #   | Règle                                                                                                                                                                                                      | Rendus     | Statut |
| --- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------- | ------ |
| 4.1 | **Web : fit dynamique sur 1 ligne** (mesuré), plafonné à **10** + bouton **« … »** qui déplie tout. Le « … » est un bouton, pas une troncature figée.                                                      | full-web   | ✅     |
| 4.2 | **Print : TOUTES les technos** s'affichent (multi-lignes), pas de « … ».                                                                                                                                   | full-print | ✅     |
| 4.3 | Sur tablette le rendu paraît « sans max » : la colonne étroite fait tenir < 10 pastilles, donc le plafond de 10 ne mord pas. Sur desktop large il coupe à 10 (ajustable via `MAX_VISIBLE_WHEN_COLLAPSED`). | full-web   | ✅     |
...
| #   | Règle                                                                                    | Rendus      | Statut                    |
| --- | ---------------------------------------------------------------------------------------- | ----------- | ------------------------- |
| 5.1 | **Le CV court DOIT tenir sur 1 page A4** à l'impression. Son 0.9 / 0.4 est tuné pour ça. | short-print | 🧪 `short-cv-one-page`    |
| 5.2 | Rythme inter-sections **uniforme** (même écart avant chaque titre de section).           | full-print  | 🧪 `print-section-rhythm` |
...
| #   | Règle                                                                                                          | Rendus     | Statut |
| --- | -------------------------------------------------------------------------------------------------------------- | ---------- | ------ |
| 7.1 | **CV court** : lien « voir le CV complet en ligne » (court → complet), visible dans tous les régimes.          | short      | ✅     |
| 7.2 | **CV complet** : lien « CV en ligne » (full-PDF → full-web), **print-only** (masqué à l'écran, on y est déjà). | full-print | ✅     |
...
1. Lancer le dev server et **vérifier les 4 rendus + le mobile** (Playwright), pas un
   seul — cf. [checklist détaillée](cv-rendering-review-checklist.md).
