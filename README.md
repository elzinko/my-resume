...
CV personnel en [Next.js](https://nextjs.org/) et [Tailwind CSS](https://tailwindcss.com/docs/). Les contenus multilingues vivent dans un **fichier unique** [`data/cv/bundle.json`](data/cv/bundle.json) (clés `fr` et `en`). Le catalogue pour le matching (ids Dato + tokens texte) est **dérivé du même fichier** au chargement côté serveur (`lib/match-catalog-from-bundle.ts`), sans artefact JSON séparé.
