...
The `contract` parameter controls the display of the Malt freelance link:

- `contract=cdi` -- hides the Malt link (CDI/permanent position).
- `contract=freelance` -- shows it (default behavior when omitted).
...
| `data/offers/*.ts` | Pre-defined offer catalogs |
