...
export default function logoMalt({ onNavigate }: LogoMaltProps) {
  return (
    <a
      ...
      className={cvHeaderIconBtn.malt}
      target="_blank"
      rel="noopener noreferrer"
      aria-label="Malt"
      onClick={() => onNavigate?.()}
    >
      <span className="inline-flex transition-all duration-200 [&_svg]:h-[var(--cv-toolbar-icon)] [&_svg]:w-[var(--cv-toolbar-icon)] [&_svg]:grayscale-[0.35] group-hover:[&_svg]:grayscale-0">
        <svg
