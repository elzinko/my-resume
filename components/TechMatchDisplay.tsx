...
interface TechMatchDisplayProps {
  ...
  lang: 'fr' | 'en';
  /**
   * `true` : pastilles critère + années sur une ligne (CV court / colonne étroite).
   * `false` : colonnes côte à côte + cartes (offre pleine largeur).
   */
  condensed?: boolean;
}
      ...
      <div className="flex w-full flex-col print:flex-row print:space-x-4 md:flex-row md:space-x-6">
        {entries.map((entry, index) => {
          ...
          return (
            <div
              key={`${index}-${entry.label}`}
              className={`cv-match-requirement-card mt-4 min-w-0 flex-1 print:mt-2 ${
                !hasMatches && !entry.yearsFromOverride ? 'opacity-50' : ''
              }`}
              style={{ breakInside: 'avoid' }}
              data-testid="profile-match-entry"
            >
              <div className="mt-0 flex items-baseline justify-between gap-3 print:mt-0">
                <h3 className="min-w-0 flex-1 text-sm font-semibold leading-snug text-orange-300 print:text-[10px] print:!text-orange-300">
                  {entry.label}
                </h3>
                {yearsLabel ? (
                  <span className="cv-pill-match-metric shrink-0 px-2 py-0.5 text-sm print:px-1.5 print:py-0.5 print:text-[10px]">
                    {yearsLabel}
                  </span>
                ) : null}
              </div>
              ...
              {hasMatches ? (
                ...
                <p className="mt-2 text-xs text-gray-500 print:mt-1 print:text-[10px] md:text-sm">
                  {t.manualYearsHint}
                </p>
              ) : (
